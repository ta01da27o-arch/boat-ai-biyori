...
// 過去傾向データ（ダミー、0～100）
// ===============================
const pastTrend = Array.from({length:24},()=>[60,50,45,40,35,30]);

// ===============================
// コース有利補正（ボートレース寄り）
// ===============================
const courseBonus = [20,10,0,-5,-10,-15]; // 1～6コース補正
...
function calcAllWithTrend(stadiumIndex){
  ...
  for(let i=0;i<6;i++){
    const b = Math.floor(40 + Math.random()*40);   // 実績
    const p = Math.floor(35 + Math.random()*45);   // 予測
    const t = trend[i];                             // 過去傾向
    const bonus = courseBonus[i];                   // コース補正
    const a = Math.round(b*0.3 + p*0.5 + t*0.2 + bonus); // AIスコア
...
function updateExpectationBars(base,predict,ai){
  ...
  document.querySelectorAll(".expectation-row").forEach((row,i)=>{
    ...
    row.querySelector(".expectation-value").textContent=ai[i]+"%"; // 総合AI%右端固定
  });
}
...
function updateBets(ai){
  ...
  if(cols.length>=3){
    ...
    setCol(cols[1],[`${others[0]}-${others[1]}-${others[2]}`,`${others[0]}-${others[2]}-${others[1]}`,`${others[1]}-${others[0]}-${others[2]}`]);
    // 逃げ（1頭固定）
    const remaining=[2,3,4,5,6].filter(n=>n!==main && n!==sub && n!==third);
    setCol(cols[2],[`1-${remaining[0]}-${remaining[1]}`,`1-${remaining[0]}-${remaining[2]}`,`1-${remaining[1]}-${remaining[0]}`]);
  }
}
...
function updateTrustMeter(ai){
  ...
  if(trust<0) trust=0; if(trust>100) trust=100;
