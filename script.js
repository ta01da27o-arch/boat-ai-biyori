...
function calcAll(){
  ...
  // 過去傾向補正(A)
  const history = [5,10,8,7,6,4]; // ダミー傾向
  ai = ai.map((v,i)=>Math.round(v*0.7 + history[i]*0.3));
  ...
  // 信頼度メーター（C③）
  updateConfidenceMeter(base,predict,ai);
}
...
function getLightColor(i){ return ["#fff","#eee","#ffe5e5","#e5f0ff","#fff7cc","#e5ffe5"][i]; }
...
function updateBets(ai){
  ...
  const main=sorted[0].i, sub=sorted[1].i, third=sorted[2].i;
...
function setCol(col,arr){
  col.querySelectorAll(".bet-item").forEach((el,i)=>el.textContent=arr[i]||"");
}
...
function updateHoleBets(ai){
  const sorted=ai.map((v,i)=>({v,i:i+1})).sort((a,b)=>b.v-a.v);
  const holeCandidates=[1,2,3,4,5,6].filter(n=>!sorted.slice(0,3).map(s=>s.i).includes(n));
  const holeArr=[
    `${holeCandidates[0]}-${holeCandidates[1]}-${holeCandidates[2]}`,
    `${holeCandidates[1]}-${holeCandidates[2]}-${holeCandidates[0]}`,
    `${holeCandidates[2]}-${holeCandidates[0]}-${holeCandidates[1]}`
  ];
  const holeCol=document.querySelectorAll(".bet-col")[3];
  setCol(holeCol,holeArr);
}
...
function updateHitRateSimulation(base,predict,ai){
  ...
  rows.forEach((row,i)=>{
    ...
    value.style.width="50px"; value.style.textAlign="right"; value.style.marginRight="8px";
    barOuter.style.flex="1"; barOuter.style.height="14px"; barOuter.style.border="1px solid #333";
    barOuter.style.background=lightColors[i]; barOuter.style.borderRadius="4px";
    bar.style.height="100%"; bar.style.width=hitRate+"%"; bar.style.background=colors[i];
    bar.style.border="1px solid #000"; bar.style.boxSizing="border-box";
  });
}
...
function updateConfidenceMeter(base,predict,ai){
  ...
  if(!meter) return;
  const avgAI=Math.round(ai.reduce((s,v)=>s+v,0)/ai.length);
  const variation=Math.round(Math.max(...ai)-Math.min(...ai));
  const confidence=Math.max(0, Math.min(100, avgAI-variation/2));
  meter.textContent=`信頼度 ${confidence}%`;
}
