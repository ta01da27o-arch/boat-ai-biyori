...
function updateAnalysis(ai){
  ...
  if(main === 1){
    text += "スタートから1コースが先手を取り主導権。イン逃げ濃厚な流れ。\n";
  }else if(main <= 3){
    text += `${main}コースが鋭く攻めて主導権を握る展開。\n`;
  }else{
    text += `${main}コースが外から一気に仕掛け波乱含みの流れ。\n`;
  }

  text += `${sub}コースが巧みに続き連対有力。\n`;

  text += `${third}コースも展開次第で食い込み可能。\n`;

  text += `${low1}コース、${low2}コースは伸び欠き苦戦気配。\n`;

  text += `軸は${main}コース中心、相手筆頭は${sub}コース。`;
