...
function calcAllWithTrend(stadiumIndex){
  ...
  for(let i=0;i<6;i++){
    // 決まり手補正（1コースは逃げ有利、外枠は少し不利）
    let kimariteBonus = i===0 ? 10 : i>=3 ? -5 : 0;
    // 基本計算式：実績+予測+過去傾向+決まり手補正
    const b = Math.floor(40 + Math.random()*40);
    const p = Math.floor(35 + Math.random()*45);
    const t = trend[i];
    const a = Math.round(b*0.3 + p*0.4 + t*0.2 + kimariteBonus*0.1);
...
function updateExpectationBars(base,predict,ai){
  ...
  document.querySelectorAll(".expectation-row").forEach((row,i)=>{
    ...
    [base[i],predict[i],ai[i]].forEach((val,j)=>{
      ...
      outer.style.marginRight="40px"; // 右端に固定スペース
...
function updateBets(ai){
  ...
  const main3 = sorted.slice(0,3).map(x=>x.i);
  const others = [1,2,3,4,5,6].filter(n=>!main3.includes(n));

  const cols=document.querySelectorAll(".bet-col");
  if(cols.length>=3){
    // 本命
    setCol(cols[0],[
      `${main3[0]}-${main3[1]}-${main3[2]}`,
      `${main3[0]}-${main3[2]}-${main3[1]}`,
      `${main3[1]}-${main3[0]}-${main3[2]}`
    ]);
    // 対抗
    setCol(cols[1],[
      `${main3[1]}-${main3[2]}-${others[0]}`,
      `${main3[2]}-${main3[1]}-${others[1]}`,
      `${main3[1]}-${main3[0]}-${others[2]}`
    ]);
    // 逃げ（常に1コース固定）
    setCol(cols[2],[
      `1-${others[0]}-${others[1]}`,
