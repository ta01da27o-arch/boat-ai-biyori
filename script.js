...
function updateExpectationBars(base,predict,ai){
  ...
  document.querySelectorAll(".expectation-row").forEach((row,i)=>{
    ...
    row.className = "expectation-row c"+(i+1); // 各コースクラス付与
    ...
    const makeLine=(val,label)=>{
      ...
      bar.className = "attack-"+label.toLowerCase();
...
function updateHitRate(ai){
  ...
  const trials = 1000;
  let hit = Array(6).fill(0); // コース1〜6着的中数

  const sumAI = ai.reduce((a,b)=>a+b,0);
  const probs = ai.map(v=>v/sumAI);

  for(let t=0;t<trials;t++){
    let pool = [0,1,2,3,4,5]; //インデックス0〜5
    let finish = [];
    ...
    // 着別集計
    finish.forEach((c,idx)=>{ hit[c]++; });
  }

  const textDiv = document.getElementById("hitRateText");
  textDiv.innerHTML = hit.map((v,i)=>`${i+1}コース: ${(v/trials*100).toFixed(1)}%`).join("<br>");
}