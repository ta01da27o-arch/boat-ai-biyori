...
function updateRaceTypeByAI(ai){
 ...
 if(inner>middle+10&&inner>outer+15) type="イン有利型";
 else if(middle>inner&&middle>outer) type="中枠攻め型";
 else if(outer>inner&&outer>middle) type="外伸び型";
...
function updateBets(ai){
 ...
 const others=[1,2,3,4,5,6].filter(n=>![a,b,c].includes(n));

 const cols=document.querySelectorAll(".bet-col");

 if(cols[0]) setCol(cols[0],[`${a}-${b}-${c}`,`${a}-${c}-${b}`,`${b}-${a}-${c}`]);
 if(cols[1]) setCol(cols[1],[`${b}-${c}-${others[0]}`,`${b}-${c}-${others[1]}`,`${b}-${c}-${others[2]}`]);
 if(cols[2]) setCol(cols[2],[`1-${a}-${b}`,`1-${a}-${c}`,`1-${b}-${c}`]);
}
